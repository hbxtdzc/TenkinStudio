...
public class UIScrollView : MonoBehaviour
{
    ...
    public System.Action<OverBoundary> onOverBoundary;
    ...
    public void ShowSpecialOne(GameObject specialGO)
    {
        if (specialGO.activeInHierarchy)
        {
            mCalculatedBounds = false;
            Bounds bound = bounds;
            Vector3 position = specialGO.transform.localPosition;
            Vector3 offset = mPanel.clipOffset - (Vector2)position;
            Vector2 region = new Vector2(mPanel.finalClipRegion.z, mPanel.finalClipRegion.w);
            Vector2 realMin = (Vector2)bound.min + region / 2;
            Vector2 maxOffset = mPanel.clipOffset - realMin;
            ...
            {
                offset.y = 0;
                offset.x = Mathf.Clamp(offset.x, 0, maxOffset.x);
            }
            else if (movement == Movement.Vertical)
            {
                offset.x = 0;
                offset.y = Mathf.Clamp(offset.y, 0, maxOffset.y);
            }
            MoveRelative(offset);
        }
    }
    ...
    private void CheckOverBoundary()
    {
        if (onOverBoundary != null)
        {
            ...
            if (canMoveHorizontally)
            {
                if (constraint.x > 200)
                {
                    onOverBoundary(OverBoundary.Right);
                }
                else if (constraint.x < -200)
                {
                    onOverBoundary(OverBoundary.Left);
                }
            }
            ...
            if (canMoveVertically)
            {
                if (constraint.y > 200)
                {
                    onOverBoundary(OverBoundary.Up);
                }
                else if (constraint.y < -200)
                {
                    onOverBoundary(OverBoundary.Down);
                }
            }
        }
    }
	...
	void LateUpdate ()
	{
        ...
        if (mMoveSmoothToLast)
        {
            Vector2 move = mToLastOffset * delta*2;
            Vector2 realMin = (Vector2)bounds.min + region/2;
            Vector2 afterMovePos = mPanel.clipOffset - move;
            if (movement == Movement.Horizontal)
            {
                move.y = 0;
                if (afterMovePos.x <= realMin.x)
                {
                    move.x = mPanel.clipOffset.x - realMin.x;
                    mMoveSmoothToLast = false;
                    mToLastOffset = Vector3.zero;

                }
            }
            else if (movement == Movement.Vertical)
            {
                if (afterMovePos.y <= realMin.y)
                {
                    move.y = mPanel.clipOffset.y - realMin.y;
                    mMoveSmoothToLast = false;
                    mToLastOffset = Vector3.zero;
                }          
                move.x = 0;               
            ...
            {
                bool yFinish = afterMovePos.y <= realMin.y;
                bool xFinish = afterMovePos.x <= realMin.x;
                if (yFinish)
                {
                    move.y = mPanel.clipOffset.y - realMin.y;
                }
