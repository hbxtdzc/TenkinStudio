...
public class UILabel : UIWidget
{
    ...
    public string GetUrlAtWorldPosition(Vector3 worldPos)
    {
        int index = GetCharacterIndexAtPosition(worldPos);
        if (index != -1 && index < mText.Length - 6)
        {
            int leftStart;
            if (mText[index] == '[' &&
                mText[index + 1] == 'u' &&
            ...
            {
                leftStart = mText.LastIndexOf("[url=", index);
            }
            int leftEnd = mText.LastIndexOf("[/url]", index);
            if (leftStart == -1 || leftEnd > leftStart)
            {
                return null;
            }
            else
            {
                leftStart += 5;
                int linkEnd = mText.IndexOf("]", leftStart);
                if (linkEnd != -1 ) return mText.Substring(leftStart, linkEnd - leftStart);
            }
        }
        return null;
    }
