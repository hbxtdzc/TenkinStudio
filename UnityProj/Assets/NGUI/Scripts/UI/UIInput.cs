...
public class UIInput : MonoBehaviour
{
    ...
    public delegate void SelectStateChange(bool select);

    public SelectStateChange onSelectChange;
	...
	public string value
	{
		...
		set
		{
			...
			{
				mKeyboard.text = Validate(value);
				mCached = value;
                UpdateLabel();
                TGameCore.clientOutputLog("UIInput UpdateLabel", 0);
			}
#endif
            if (mValue != value)
			{
	...
	public string Validate (string val)
	{
	    ...
	    {
            int limit = 0;
            char[] leftArr = val.ToCharArray();
            length = 0;
            for (int i = 0; i < leftArr.Length; i++)
            {
                int len = System.Text.Encoding.UTF8.GetByteCount(leftArr[i].ToString());
                if (len == 3)
                {
                    len = 2;
                }
	...
	protected virtual void Insert (string text)
	{
		...
		StringBuilder sb = new StringBuilder(rightLength + leftLength + textLength);
		sb.Append(left);
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
            ...
            if (characterLimit > 0 && (leftLength + rightLength + i) >= characterLimit) break;
	...
	public void UpdateLabel ()
	{
		if (label != null)
		{
		    ...
		    int length = GetLength(fullText);
		    if (characterLimit > 0 && length > characterLimit)
		    {
		        int limit = 0;
                char[] leftArr = fullText.ToCharArray();
		        length = 0;
		        for (int i = 0; i < leftArr.Length; i++)
		        {
		            int len = System.Text.Encoding.UTF8.GetByteCount(leftArr[i].ToString());
